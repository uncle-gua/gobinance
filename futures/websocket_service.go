...
func WsAggTradeServe(symbol string, handler WsAggTradeHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	...
	return wsServe(cfg, wsHandler, errHandler, logger)
}

// WsCombinedAggTradeServe is similar to WsAggTradeServe, but it handles multiple symbols
func WsCombinedAggTradeServe(symbols []string, handler WsAggTradeHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func wsMarkPriceServe(endpoint string, handler WsMarkPriceHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
...
func WsMarkPriceServe(symbol string, handler WsMarkPriceHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := fmt.Sprintf("%s/%s@markPrice", getWsEndpoint(), strings.ToLower(symbol))
	return wsMarkPriceServe(endpoint, handler, errHandler, logger)
}

// WsMarkPriceServeWithRate serve websocket that pushes price and funding rate for a single symbol and rate.
func WsMarkPriceServeWithRate(symbol string, rate time.Duration, handler WsMarkPriceHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	var rateStr string
	switch rate {
	case 3 * time.Second:
		rateStr = ""
...
func wsAllMarkPriceServe(endpoint string, handler WsAllMarkPriceHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
...
func WsAllMarkPriceServe(handler WsAllMarkPriceHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := fmt.Sprintf("%s/!markPrice@arr", getWsEndpoint())
	return wsAllMarkPriceServe(endpoint, handler, errHandler, logger)
}

// WsAllMarkPriceServeWithRate serve websocket that pushes price and funding rate for all symbol and rate.
func WsAllMarkPriceServeWithRate(rate time.Duration, handler WsAllMarkPriceHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	var rateStr string
	switch rate {
	case 3 * time.Second:
		rateStr = ""
...
func WsKlineServe(symbol string, interval string, handler WsKlineHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsCombinedKlineServe(symbolIntervalPair map[string]string, handler WsKlineHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := getCombinedEndpoint()
...
func WsMiniMarketTickerServe(symbol string, handler WsMiniMarketTickerHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsAllMiniMarketTickerServe(handler WsAllMiniMarketTickerHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsMarketTickerServe(symbol string, handler WsMarketTickerHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsAllMarketTickerServe(handler WsAllMarketTickerHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsBookTickerServe(symbol string, handler WsBookTickerHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsAllBookTickerServe(handler WsBookTickerHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsLiquidationOrderServe(symbol string, handler WsLiquidationOrderHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsAllLiquidationOrderServe(handler WsLiquidationOrderHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func wsPartialDepthServe(symbol string, levels int, rate *time.Duration, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	if levels != 5 && levels != 10 && levels != 20 {
		return nil, errors.New("invalid levels")
	}
	levelsStr := fmt.Sprintf("%d", levels)
	return wsDepthServe(symbol, levelsStr, rate, handler, errHandler, logger)
}

// WsPartialDepthServe serve websocket partial depth handler.
func WsPartialDepthServe(symbol string, levels int, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	return wsPartialDepthServe(symbol, levels, nil, handler, errHandler, logger)
}

// WsPartialDepthServeWithRate serve websocket partial depth handler with rate.
func WsPartialDepthServeWithRate(symbol string, levels int, rate time.Duration, logger *Logger, handler WsDepthHandler, errHandler ErrHandler) (done chan struct{}, err error) {
	return wsPartialDepthServe(symbol, levels, &rate, handler, errHandler, logger)
}

// WsDiffDepthServe serve websocket diff. depth handler.
func WsDiffDepthServe(symbol string, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	return wsDepthServe(symbol, "", nil, handler, errHandler, logger)
}

// WsCombinedDepthServe is similar to WsPartialDepthServe, but it for multiple symbols
func WsCombinedDepthServe(symbolLevels map[string]string, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := getCombinedEndpoint()
...
func WsCombinedDiffDepthServe(symbols []string, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := getCombinedEndpoint()
	for _, s := range symbols {
...
func WsDiffDepthServeWithRate(symbol string, rate time.Duration, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	return wsDepthServe(symbol, "", &rate, handler, errHandler, logger)
}

func wsDepthServe(symbol string, levels string, rate *time.Duration, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	var rateStr string
	if rate != nil {
		switch *rate {
...
func WsBLVTInfoServe(name string, handler WsBLVTlogger, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsBLVTKlineServe(name string, interval string, handler WsBLVTKlineHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsCompositiveIndexServe(symbol string, handler WsCompositeIndexHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsUserDataServe(listenKey string, handler WsUserDataHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
