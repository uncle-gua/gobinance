...
func WsPartialDepthServe(symbol string, levels string, handler WsPartialDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := fmt.Sprintf("%s/%s@depth%s", getWsEndpoint(), strings.ToLower(symbol), levels)
	return wsPartialDepthServe(endpoint, symbol, handler, errHandler, logger)
}

// WsPartialDepthServe100Ms serve websocket partial depth handler with a symbol, using 100msec updates
func WsPartialDepthServe100Ms(symbol string, levels string, handler WsPartialDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func wsPartialDepthServe(endpoint string, symbol string, handler WsPartialDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsCombinedPartialDepthServe(symbolLevels map[string]string, handler WsPartialDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsDepthServe(symbol string, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := fmt.Sprintf("%s/%s@depth", getWsEndpoint(), strings.ToLower(symbol))
	return wsDepthServe(endpoint, handler, errHandler, logger)
}

// WsDepthServe100Ms serve websocket depth handler with a symbol, using 100msec updates
func WsDepthServe100Ms(symbol string, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func wsDepthServe(endpoint string, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
		j, err := newJSON(message)
		if err != nil {
...
func WsCombinedDepthServe(symbols []string, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := getCombinedEndpoint()
	for _, s := range symbols {
		endpoint += fmt.Sprintf("%s@depth", strings.ToLower(s)) + "/"
	}
	endpoint = endpoint[:len(endpoint)-1]
	return wsCombinedDepthServe(endpoint, handler, errHandler, logger)
}

func WsCombinedDepthServe100Ms(symbols []string, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := getCombinedEndpoint()
	for _, s := range symbols {
...
func wsCombinedDepthServe(endpoint string, handler WsDepthHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	cfg := newWsConfig(endpoint)
	wsHandler := func(message []byte) {
		j, err := newJSON(message)
		if err != nil {
...
func WsTradeServe(symbol string, handler WsTradeHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsCombinedTradeServe(symbols []string, handler WsCombinedTradeHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := getCombinedEndpoint()
	for _, s := range symbols {
...
func WsCombinedMarketStatServe(symbols []string, handler WsMarketStatHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
	endpoint := getCombinedEndpoint()
	for s := range symbols {
...
func WsMarketStatServe(symbol string, handler WsMarketStatHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsAllMarketsStatServe(handler WsAllMarketsStatHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsAllMiniMarketsStatServe(handler WsAllMiniMarketsStatServeHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
...
func WsCombinedBookTickerServe(symbols []string, handler WsBookTickerHandler, errHandler ErrHandler, logger *Logger) (done chan struct{}, err error) {
